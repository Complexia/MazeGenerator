...
public class NormalMaze extends Maze {
	...
	public void initMaze(int rs, int cs, int entR, int entC, int exitR, int exitC, List<int[]> tunnelList) {
		...
		// initialise the map from the bottom left corner
		for (int i = 0; i < sizeR; i++){
			for (int j = 0; j < sizeC; j++) {
				Cell cell = new Cell(i, j);
				map[i][j] = cell;
				// rectangular cells only have walls/neighbors indexed by 0 (EAST), 2 (NORTH), 3 (WEST), 5 (SOUTH)
				for (int k = 0; k < 3; k++) {
					if (k == 1)
						continue;				
					cell.wall[k] = new Wall();	// as we grow the map bottom-up, left-right, EAST (k=0) and NORTH (k=2) walls did not exist before and need to be created new
				}
				for (int k = 3; k < NUM_DIR; k++) {
					...
					if (isIn(i + deltaR[k], j + deltaC[k])) {
						...
						cell.wall[k] = neigh.wall[oppoDir[k]];	// WEST and SOUTH walls might have already been created by previous cells
						cell.neigh[k] = neigh;					// create the WEST/SOUTH neighbors for cell
						neigh.neigh[oppoDir[k]] = cell;			// make cell the neighbor of its neighbor as well
					}
					else
						cell.wall[k] = new Wall();				// if WEST and SOUTH walls do not exist, create new walls (boundary cases)
				}
			}
		}
	...
	/**
	 * This checks if the created maze is perfect: (1) a BFS starting from the entrance can reach all possible cells within the maze
	 * 										   and (2) the created maze has no cycles
	 */
